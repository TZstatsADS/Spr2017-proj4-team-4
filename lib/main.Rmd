...
python_paper <- read.csv('dd5.csv',skip = 1,header = FALSE)
python_paper <- python_paper$V1
...
AKumar$Paper <- gsub("<","",python_paper)
...
dim(dtm_train_tfidf)
dtm <- data.frame(as.matrix(dtm_train_tfidf))
...
pca <- prcomp(dtm_train_df, center=TRUE, scale=TRUE)
summary(pca)

## Find the number of principal components that captures 99% of the variance.
lv <- cumsum(pca$sdev)/sum(pca$sdev)
idx_99 <- min(which((lv > 0.99^2)==TRUE))

pca_imps <- as.data.frame(pca$x[,1:idx_99])
write.csv(pca_imps, file="pca.csv")
```
